...
export default function Home() {
  ...
  const defaultDataUrl =
    "https://api.open-meteo.com/v1/forecast?latitude=-23.55&longitude=-46.64&hourly=temperature_2m,weathercode&models=jma_seamless&daily=weathercode,temperature_2m_max,temperature_2m_min&current_weather=true&precipitation_unit=inch&timeformat=unixtime&timezone=America%2FSao_Paulo";

  const getData = useCallback(async (url1: string, url2: string) => {
    ...
    const weatherData: WeatherData = {
      ...
      forecast5days: {
        ...
        tempMax: weatherDataApi.daily.temperature_2m_max.map(
          (item: number[]) => item
        ),
        tempMin: weatherDataApi.daily.temperature_2m_min.map(
          (item: number[]) => item
        ),
        weathercode: weatherDataApi.daily.weathercode.map(
          (item: number) => item
        ),
      },
      forecastNextHours: {
        timestamp: weatherData24hours.hourly.time.map((item: number[]) => item),
        temp: weatherData24hours.hourly.temperature_2m.map(
          (item: number[]) => item
        ),
        weathercode: weatherData24hours.hourly.weathercode.map(
          (item: number[]) => item
        ),
      },
    };
  ...
  return (
    ...
    >
      <div className="max-w-[1040px] display1:hidden h-screen m-auto p-2 w-full flex flex-col place-content-between pt-8 pb-8 pl-12 pr-12">
        <div className="w-full  justify-end items-center space-x-4 flex flex-row">
          <span className="text-white font-medium text-[26px] block text-right">
            {weatherData?.date}
          </span>
          <div className="w-[1px] h-[24px] bg-line"></div>
          <span className="text-white font-medium text-[26px]">
            {hour}:{minutes}
          </span>
        </div>
        <div className="w-full max-h-[350px] h-max p-2 space-y-8 ">
          <span className="font-bold text-white font-sans text-[76px] block text-right">
            {weatherData ? weatherCodes[weatherData.weathercode] : null}
          </span>
          <div className="w-full m-auto border-[0px] border-line "></div>
          <motion.div className="h-max flex space-x-2 justify-end">
            {weatherData?.forecastNextHours ? (
              <NexthHours forecastNextHours={weatherData?.forecastNextHours} />
            ) : null}
          </motion.div>
        </div>
      </div>
      {weatherData && (
        <Sidebar
          ...
          forecast={weatherData.forecast5days}
        />
      )}
    </div>
  );
